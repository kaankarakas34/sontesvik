...
async function testMultiIncentiveAPIFixed() {
  ...
  try {
    console.log('=== Multi-Incentive Application API Test (Fixed) ===\n');
    ...
    // Use first two incentives for testing - keep as UUID strings
    ...
    console.log('✓ Using incentive IDs for test (as UUID strings):', testIncentiveIds);
    console.log('');
    
    // 2. Test the exact same request that frontend makes (FIXED)
    ...
    const applicationData = {
      ...
      incentiveIds: testIncentiveIds // UUID string'leri olduğu gibi gönder - INTEGER'A ÇEVİRME!
    };
    ...
    console.log('Application ID:', createResponse.data.data?.id);
    console.log('Incentives count:', createResponse.data.data?.incentives?.length || 0);
